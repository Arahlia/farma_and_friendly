...
});" class="flex flex-col transition-all h-full overflow-hidden pt-4 pl-4 pr-4">
        ...
        <ul class="p-2 grid w-full spacey-y-2">
            ...
            <li id="conversation-{{$conversation->id}}" wire:key="{{$conversation->id}}" class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 {{$conversation->id==$selectedConversation?->id ? 'bg-gray-100/70':''}}">
                <a href="#" class="shrink-0">
                    <x-avatar src="{{ $conversation->getReceiver()->photo ? asset('img/' . $conversation->getReceiver()->photo) : 'default-avatar.jpg' }}" />
                </a>
                ...
                <aside class="grid grid-cols-12 w-full">
                    ...
                    <a href="{{route('chat',$conversation->id)}}" class="col-span-11 border-b pb-2 border-gray-200 relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
                        ...
                        <div class="flex justify-between w-full items-center">
                            ...
                            <h6 class="truncate font-medium tracking-wider text-2xl text-gray-900">
                                {{$conversation->getReceiver()->name}}
                            </h6>

                            <small class="text-gray-700 text-base">{{$conversation->messages?->last()?->created_at?->shortAbsoluteDiffForHumans()}} </small>
                        ...
                        <div class="flex gap-x-2 items-center">
                            ...
                            <p class="grow truncate text-lg font-[100]">
                                {{$conversation->messages?->last()?->body??' '}}
                            </p>

                            {{-- unread count --}}
                            @if ($conversation->unreadMessagesCount()>0)
                            <span class="font-bold p-px px-2 text-lg shrink-0 rounded-full bg-customGreen-500 text-white">
                        ...
                        <x-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                ...
                                <div class="w-full p-1">
                                    ...
                                    <button wire:click="viewProfile({{ $conversation->getReceiver()->id }})" class="items-center gap-3 flex w-full px-4 py-2 text-left text-xl leading-5 text-gray-500 hover:bg-gray-100 transition-all duration-150 ease-in-out focus:outline-none focus:bg-gray-100">
                                    ...
                                    <button onclick="confirm('¿Estás seguro?')||event.stopImmediatePropagation()" wire:click="deleteByUser('{{encrypt($conversation->id)}}')" class="items-center gap-3 flex w-full px-4 py-2 text-left text-xl leading-5 text-gray-500 hover:bg-gray-100 transition-all duration-150 ease-in-out focus:outline-none focus:bg-gray-100">
